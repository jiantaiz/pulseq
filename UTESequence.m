...
fov=250e-3; Nx=32;             % Define FOV and resolution,m
alpha=90;                       % flip angle
sliceThickness=3e-3;            % slice
TR=50e-3;                       % TR
Nr=36;                         % number of radial spokes
...
ro_asymmetry=0;              % 0: fully symmetric 1: half-echo
minRF_to_ADC_time=100e-6;        % the parameter wich defines TE (together with the RO asymmetyry)
...
for i=1:Nr
    for c=1:2
        ...
        gradient_p(i)=gx.amplitude*sin(phi);
        gradient_r(i)=gx.amplitude*cos(phi);
        gradient_pre(i)=gxPre.amplitude;
        gradient_spoil(i)=gxSpoil.amplitude;
        rf_phaseOffset(i)= rf.phaseOffset/pi*360;
        adc_phaseOfffset(i)=adc.phaseOffset/pi*360;
